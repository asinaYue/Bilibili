      ...
      <div class="search-container">
        <form class="search-form">
          <div class="search-content">
            <input type="text" placeholder="你所热爱的就是你的生活" />
          </div>
...
export default {
  ...
  setup() {
    ...
    const mousemoveHandler = (e) => {
      x_new.value = e.clientX;
      x_offset.value = x.value - x_new.value;
      for (const key in images) {
        if (images.hasOwnProperty(key)) {
          let level = (6 - parseInt(key)) * 10;
          const element = images[key];
          const element_data = data_images[key];
          let b_new = Math.abs(element_data.b + x_offset.value / step);
          let l_new = 0 - x_offset.value / level;
    ...
    onMounted(() => {
      ...
      banner.addEventListener('mouseout', mouseoutHandler);
    });
  },
};
</script>
